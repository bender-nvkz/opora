...
### Общие принципы
- **DRY**: повторяющаяся логика выносится в абстракцию. Если одно и то же
  написано дважды — это сигнал к рефакторингу, не норма.
- **KISS**: простейшее решение которое работает правильно. Сложность
  добавляется только если простое решение объективно не справляется.
- **YAGNI**: не реализовывать то чего нет в текущей спеке "на будущее".
  Задел — только если явно указан в спеке или ADR.

### Стандарты стека
- PHP 8.4: использовать property hooks, readonly классы, named arguments
  где они улучшают читаемость — не избегать новых фич без причины.
- Yii3 DI: tagged iterators для коллекций (middleware, health checks,
  module installers) — не хардкодить списки в конфигах.
- Cycle ORM: mapper-паттерн, не Active Record. Entity не знает об ORM.

### Инженерная культура
- **Не экономить на решениях**: если правильное решение требует
  дополнительного интерфейса или слоя — делать правильно, не срезать углы
  ради краткости реализации.
- **Комплексный анализ**: перед реализацией проверять смежные места
  которые затрагивает изменение. Не туннельное зрение на текущий файл.
- **Явные решения**: любое нетривиальное решение сопровождается
  объяснением почему именно так. Код без контекста — технический долг.
